...
class Cpu(network: Network, gridX: Int, gridY: Int) : Chip(network, gridX, gridY) {
    ...
    */
    ...
    private fun createBitmap(): Bitmap? {
        ...
        actualRect = calculateActualRect()?.makeSquare()?.scale(2.5f * currentZoom)
        actualRect?.let {
            bitmap =
                Bitmap.createScaledBitmap(network.gameView.cpuImage, it.width(), it.height(), true)
        }
        return bitmap
    }

    override fun display(canvas: Canvas, viewport: Viewport) {
        val newZoom = viewport.userScale

        // Recreate bitmap if zoom changed
        if (bitmap == null || newZoom != currentZoom) {
            currentZoom = newZoom
            bitmap = createBitmap()
        }
        ...
        actualRect?.let { rect ->
            ...
            val paint = Paint()
            bitmap?.let { canvas.drawBitmap(it, null, rect, paint) }

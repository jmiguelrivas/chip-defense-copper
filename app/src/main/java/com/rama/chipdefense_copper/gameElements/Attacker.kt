...
import com.rama.chipdefense_copper.effects.Explodable
import com.rama.chipdefense_copper.effects.Fadable
import com.rama.chipdefense_copper.effects.Fader
import com.rama.chipdefense_copper.networkmap.*
...
open class Attacker(
    ...
    var attackerData = Data(
            representation = representation, number = number, binaryDigits = 0, hexDigits = 0,
    ...
    var displacement =
        Pair(Random.nextInt(5) - 1, Random.nextInt(7) - 2) // small shift in display to avoid over-crowding on the screen
    ...
    {
        val newAttacker =
            Attacker(network = network, representation = attackerData.representation, number = attackerData.number, speed = data.speed)
             ...
             */
    {
        if (attackerData.representation == Representation.UNDEFINED)
            attackerData.representation =
                if (attackerData.number >= 32u) Representation.HEX else Representation.BINARY
            ...
            /** @return possible bonus on kill due to hero */
    {
        ...
        val extraCash =
            Random.nextFloat() * strength * 2.0f // this gives an expectation value of 'strength'
            ...
            /** temporarily decrease the attacker's speed, e.g. due to a resistor effect */
    {
        ...
        var additionalDuration =
            GameMechanics.resistorBaseDuration / data.speedModifier * network.gameMechanics.heroModifier(Hero.Type.INCREASE_CHIP_RES_DURATION)
    ...
    {
        return if (attackerData.representation == Representation.BINARY) attackerData.number.toString(radix = 2)
            .padStart(attackerData.binaryDigits, '0')
        else "x" + attackerData.number.toString(radix = 16).uppercase()
            .padStart(attackerData.hexDigits, '0')
    }
    ...
    fun createBitmap(text: String) {
        ...
        when (attackerData.representation) {
            Representation.BINARY -> {
                ...
                textPaint.color = if (data.speedModificationTimer > 0)
                    resources.getColor(R.color.attackers_slowed_hex)
                else
                    resources.getColor(R.color.attackers_foreground_hex)
        ...
        numberBitmap =
            Bitmap.createBitmap(bounds.width() + 16, bounds.height() + 16, Bitmap.Config.ARGB_8888)
        ...
        // glow effect
        val alphaBitmap = numberBitmap.extractAlpha()
    ...
    override fun display(canvas: Canvas, viewport: Viewport) {
        ...
        if (currentZoom != viewport.userScale) {
            currentZoom = viewport.userScale
            makeNumber() // regenerates bitmap with new zoom
        }
        ...
        if (animationCount > 0)
            oldNumberBitmap?.let {
                ...
                val oldSource =
                    Rect(0, numberBitmap.height - divider, numberBitmap.width, numberBitmap.height)
    ...
    fun onDown(event: MotionEvent): Boolean {
        ...
        )  // do not allow tapping on objects HELD or GONE
        {
    ...
    fun logState() {
        ...
        logger?.debug(
                "%s: Attacker [%s] on track %d, link %d, speed %f, going from %s to %s, distance travelled is %f, still %f to go. State is %s".format(
                        this.hashCode().toString(radix = 16), numberAsString(),
                        onTrack?.data?.ident ?: -1, onLink?.data?.ident
                            ?: -1, data.speed, startNode?.data?.ident ?: -1, endNode?.data?.ident
                            ?: -1,

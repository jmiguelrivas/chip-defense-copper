...
class Stage(var gameMechanics: GameMechanics, var gameView: GameView) {
    ...
    fun createNewAttacker(
        maxNumber: Int, speed: Float, isCoin: Boolean = false,
        ...
        val attacker = if (isCoin)
            ...
            val strength =
                Random.nextFloat() * (maxNumber + 1) * gameMechanics.heroModifier(Hero.Type.DECREASE_ATT_STRENGTH)
    ...
    fun createChip(
        ...
        gridY: Int,
        ident: Int = -1,
        type: Chip.ChipType = Chip.ChipType.EMPTY
    ): Chip
    ...
    fun createLink(
        ...
        ident: Int,
        mask: Int = 0xF,
        variant: Link.Variant = Link.Variant.CONVEX
    ): Link?
             ...
             * */
    {
        ...
        if (network.links.containsKey(ident)) {  // link already present, do not create a new one
    ...
    fun createWave(
        ...
        ticksUntilFirstAttacker =
            (4 * 20).toLong()  // after first wave, set a delay here. 20 ticks equal one second
